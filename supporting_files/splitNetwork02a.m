function [n1,n2,idxn1a,idxn2a,nB,nb1,nb2,idxB] = splitNetwork02a(G,cc)
%
...
line_data.I = G.Edges.EndNodes(:,1);
line_data.J = G.Edges.EndNodes(:,2);

idx1a = ismember(line_data.I,n1);
idx2a = ismember(line_data.J,n2);

idxa = idx1a & idx2a;

idx1b = ismember(line_data.I,n2);
idx2b = ismember(line_data.J,n1);
...
n1b = find(ismember(n1,nB));
n2b = find(ismember(n2,nB));
...
for lll = 1:numel(idxBa)
    ...
    idxI1 = ismember(n1,lineD.I(lll));
    idxI2 = ismember(n2,lineD.I(lll));
    idxJ1 = ismember(n1,lineD.J(lll));
    idxJ2 = ismember(n2,lineD.J(lll));
    
    
    if sum(idxI1) 
      
    boundary.n1(lll,1) = n1(idxI1);
    
    end
    
    if sum(idxJ1) 
      
    boundary.n1(lll,1) = n1(idxJ1);
    
    end
    
    if sum(idxI2)
      
    boundary.n2(lll,1) = n2(idxI2);
    
    end
    
    if sum(idxJ2) 
      
    boundary.n2(lll,1) = n2(idxJ2);
